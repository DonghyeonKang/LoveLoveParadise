...
- **출력 포트 (Output Ports)**: 출력 포트는 `<VerbNoun>Port` 형태의 이름을 사용합니다.
  - ⭕ Good: `LoadProfilePort`, `UpdateProfilePort`, `RegisterUserPort`, `DeleteManagerPort`, `LoadPasswordPort`
...
`-- useCases
    ...
    `-- <verbNoun> (유스케이스 명칭 - 예: registerUser)
        ...
        |   |   `-- out
        |   |       |-- <VerbNoun>Port.java (출력 포트 - 예: RegisterUserPort, LoadProfilePort)
...
-   `application/port/out/`: **출력 포트(Output Port)**
  ...
  -   `<VerbNoun>Port` 형태의 이름을 사용합니다.
  -   메서드 이름은 유스케이스 맥락에 따라 `operate`, `load`, `save` 등을 사용합니다.
  -   예: `LoadProfilePort.operate(UUID userId)`, `UpdateProfilePort.operate(UpdateProfileCmd cmd)`
